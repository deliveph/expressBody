<template>
    <scroller lock-x scrollbar-y use-pullup use-pulldown @on-pullup-loading="onPullupLoading" @on-pulldown-loading="refresh" v-model="status" ref="scroller">
        <div id="statistics" class="statistics_warp">
            <div class="title por">
                <router-link :to="{name:'Detail',query:{type:'use_help'}}" class="poa">使用帮助</router-link>
                <ul>
            ...
            <div class="statistics_list">
                <ul v-if="express_orders.length != 0">
                    <li v-for="(item, key) in express_orders" :key="key">
                        <div class="row">
...
<script>
import { Datetime, Scroller, Spinner, LoadMore } from 'vux'
export default {
  components: {
    Datetime,
    Scroller,
  ...
  methods: {
    onPullupLoading () {
      ...
      that.http(that.configs.apiTop + '/page/service-order-total?start_time=' + start + '&end_time=' + end + '&page=' + that.page, 'get', '', function (res) {
        let msg = res.data
        if (msg.code === 0) {
          let data = msg.data
          that.$refs.scroller.donePullup()
          that.result = data
          if (data.express_orders.length > that.n) {
            data.express_orders.pop()
          } else {
            that.$refs.scroller.disablePullup()
          }
          that.express_orders.push(...data.express_orders)
        } else {
    ...
    refresh () {
      ...
      this.http(that.configs.apiTop + '/page/service-order-total?start_time=' + start + '&end_time=' + end + '&page=' + that.page, 'get', '', function (res) {
        let msg = res.data
        if (msg.code === 0) {
          let data = msg.data
    ...
    getStatistics () {
      ...
      this.http(that.configs.apiTop + '/page/service-order-total?page=1&start_time=' + start + '&end_time=' + end, 'get', '', function (res) {
        let msg = res.data
        if (msg.code === 0) {
          let data = msg.data
          that.result = data
