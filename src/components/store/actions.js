...
function connectNim ({state, commit, dispatch}, loginInfo) {
  let {force} = Object.assign({}, loginInfo)
  // 操作为内容页刷新页面，此时无nim实例
  if (!state.nim || force) {
    if (!loginInfo.uid) {
      // 无cookie，直接跳转登录页
...
export default {
  ...
  chooseShipOrderCurrentConsigneeAddress ({
    ...
    commit('CHOOSESHIPORDERCURRENTCONSIGNEEADDRESS', consigneeAddress)
  },
  chooseShipOrderCurrentShiperAddress ({
    commit
  }, shiperAddress) {
    commit('CHOOSESHIPORDERCURRENTSHIPERADDRESS', shiperAddress)
  },
  // ###############################################
  chooseChatUser ({
    commit
